...
int grib_tool_before_getopt(grib_runtime_options* options) {
	...
	{
		/* Print a sorted list of all files in the samples dir matching *.tmpl */
		int i, count = 0;
		struct dirent **files = NULL;

		/* Get a sorted directory list */
		count = scandir(c->grib_templates_path, &files, 0, alphasort);
		if (count<0) {
			fprintf(stderr, "Could not read entries in the SAMPLES path. ");
			perror(c->grib_templates_path);
			exit(1);
		}
